"""
바이낸스 코인 과거 시세 전체 히스토리 백필(backfill) 스크립트
- data-api.binance.vision 공개 데이터 사용 (지역 차단 없음, API 키 불필요)
- BNBUSDT 상장일(2017-08-17)부터 현재까지 1시간봉 전체를 받아와
  bnb_history.csv 를 새로 생성한다 (기존 파일이 있으면 덮어씀).
- 이 스크립트는 "딱 한 번" 수동으로 실행하면 된다. 이후 매시간 자동 실행되는
  binance_bnb_fetch.py 가 이 파일 뒤에 새 데이터를 이어붙인다.

필요 패키지: requests, pandas
설치: pip install requests pandas
실행: python binance_bnb_backfill.py
"""
...
SYMBOL = "BNBUSDT"
INTERVAL = "1h"
START_DATE = "2017-08-17"  # BNBUSDT 상장일. 필요시 "2022-01-01" 등으로 조정 가능
KLINES_URL = "https://data-api.binance.vision/api/v3/klines"
HISTORY_FILE = os.path.join(os.path.dirname(__file__), "bnb_history.csv")
